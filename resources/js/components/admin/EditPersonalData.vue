...
const title = ref('Обработка персональных данных');
const seoTitle = ref('');
...
const load = async () => {
    try {
        const response = await axios.get('/api/rule-pages/personal-data');
        if (response.data && response.data.id) {
            title.value = response.data.title || 'Обработка персональных данных';
