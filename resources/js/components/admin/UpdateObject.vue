<template>
    <div class="container">
        ...
        <br>
        <v-btn color="info" block @click="updateObject()">Сохранить изменения</v-btn>
    </div>
</template>
...
const updateObject = async () => {
    try {
        await axios.post('/api/objects/update', {
            id: id,
            name: name.value,
            title: title.value,
...
const normalizeSlider = (raw) => {
    if (!raw || !Array.isArray(raw)) return [];
    return raw.map(item => {
        if (typeof item === 'string') return { url: item, alt: '', media_id: null };
        return { url: item.url || item, alt: item.alt || '', media_id: item.media_id || null };
    });
};
...
const loadObj = async () => {
    try {
        ...
        if (response.data && response.data.obj) {
            ...
            previewAlt.value = response.data.obj.preview_alt || '';
            floors.value = response.data.obj.floors || '';
            price_per_sqm.value = response.data.obj.price_per_sqm || '';
            area.value = response.data.obj.area || '';
            detail_description.value = response.data.obj.detail_description || '';
            left_image.value = response.data.obj.left_image || '';
            features.value = response.data.obj.features ? (Array.isArray(response.data.obj.features) ? response.data.obj.features : JSON.parse(response.data.obj.features)) : [];
            big_description.value = response.data.obj.big_description || '';
            slider.value = normalizeSlider(response.data.slider);
        }
    } catch (error) {
        console.error(error);
    }
